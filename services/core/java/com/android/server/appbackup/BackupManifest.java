...
class BackupManifest {
    ...
    // Combined size of the unified backup.tar (ce/ + de/ trees).
    // Stored under KEY_CE_SIZE in JSON for backward-compatibility; KEY_DE_SIZE is written as 0.
    ...
    BackupManifest(String id, String packageName, String label, String versionName,
            long versionCode, long timestampMs, long apkSize, long dataSize,
            boolean encrypted, int state, String checksum, int userId) {
        mId = id;
    ...
    void writeTo(@NonNull File dest) throws IOException {
        try {
            ...
            json.put(KEY_CE_SIZE, mDataSize);
    ...
    JSONObject toJson() throws IOException {
        try {
            ...
            json.put(KEY_TIMESTAMP, mTimestampMs);
            json.put(KEY_APK_SIZE, mApkSize);
            // Combined CE+DE tar size stored under KEY_CE_SIZE for backward-compatibility.
            // KEY_DE_SIZE and KEY_EXT_SIZE are omitted (readers use optLong default of 0).
            json.put(KEY_CE_SIZE, mDataSize);
            json.put(KEY_ENCRYPTED, mEncrypted);
            json.put(KEY_STATE, mState);
            json.put(KEY_CHECKSUM, mChecksum);
    ...
    BackupRecord toBackupRecord(@NonNull String backupDir) {
        // dataSize is the combined CE+DE tar; deDataSize is 0 in the new format.
        return new BackupRecord(mId, mPackageName, mLabel, mVersionName, mVersionCode,
                mTimestampMs, mApkSize, mDataSize, 0L, mEncrypted, mState,
                backupDir, mUserId);
    }
