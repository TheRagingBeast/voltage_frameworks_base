...
class AppDataBackupShellCommand extends ShellCommand {
    ...
    public void onHelp() {
        ...
        pw.println("  export  --package <pkg> --file <path> [--user <userId>]");
        pw.println("             Export raw backup.tar from internal storage to <path>");
        pw.println("  import  --package <pkg> --file <path> [--user <userId>]");
        pw.println("             Import raw backup.tar from <path> into internal storage");
    }
    ...
    private int cmdExport() {
        String pkg = null, file = null;
        int userId = 0;
        String opt;
        while ((opt = getNextOption()) != null) {
            switch (opt) {
                case "--package": pkg  = getNextArgRequired(); break;
                case "--file":    file = getNextArgRequired(); break;
                case "--user":    userId = Integer.parseInt(getNextArgRequired()); break;
            }
        }
        if (pkg == null || file == null) {
            getErrPrintWriter().println("--package and --file required"); return 1;
        }
        try {
            mService.exportAppBackup(pkg, userId, file);
            getOutPrintWriter().println("Exported " + pkg + " → " + file);
            return 0;
        } catch (RuntimeException e) {
            getErrPrintWriter().println("Export failed: " + e.getMessage());
            return 1;
        }
    }

    private int cmdImport() {
        String pkg = null, file = null;
        int userId = 0;
        String opt;
        while ((opt = getNextOption()) != null) {
        ...
        try {
            mService.importAppBackup(pkg, userId, file);
            getOutPrintWriter().println("Imported " + file + " → " + pkg);
            return 0;
        } catch (RuntimeException e) {
            getErrPrintWriter().println("Import failed: " + e.getMessage());
            return 1;
        }
    }
}
